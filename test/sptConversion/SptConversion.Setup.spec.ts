...
describe('SptConversion - Setup', async () => {
  ...
  describe('constructor', async () => {
    it('should have correct root set', async () => {
      const { token, vestingPoolManager, airdropContract, sptToken } = await setupTests();
    ...
    it('should revert with redeem date in the past', async () => {
      const { airdropContract, token, vestingPoolManager, sptToken } = await setupTests();
      await expect(
        airdropContract.deploy(
          token.address,
          airdropManager.address,
