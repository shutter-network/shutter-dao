...
describe('SptConversion - Claiming', async () => {
  ...
  const setupAirdrop = async (
    ...
    const { airdropContract, vestingPoolManager, token, sptToken } = await setupTest();
  ...
  const redeemAirdrop = async (
    ...
    // const {sptToken} = await setupTest();
    ...
    console.log('sptToken', sptToken.address, airdrop.address);
    await sptToken.transfer(user.address, amount);
    console.log('balance', await sptToken.balanceOf(user.address));
    await sptToken.connect(user).approve(airdrop.address, amount);
    const allowance = await sptToken.allowance(user.address, airdrop.address);
  ...
  describe('claimUnusedTokens', async () => {
    ...
    it('should revert if no tokens to claim after a vesting was created', async () => {
      const { token, sptToken } = await setupTest();
      ...
      await redeemAirdrop(airdrop, sptToken, elements, user1, amount);
