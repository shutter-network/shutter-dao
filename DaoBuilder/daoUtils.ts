...
async function getFractalContractAddressesByNetworkName() {
  ...
  const LinearERC20Voting = await import(`${contractsPath}/LinearERC20Voting.json`);
  const ModuleProxyFactory = await import(`${contractsPath}/ModuleProxyFactory.json`);
  return {
    Azorius,
    FractalRegistry,
    KeyValuePairs,
...
export const getMasterCopies = async (): Promise<{
  ...
  const { Azorius, FractalRegistry, KeyValuePairs, LinearERC20Voting, ModuleProxyFactory } =
  ...
  if (!multisendSingletonDeployment) throw new Error('Multisend contract not found');
